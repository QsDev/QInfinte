import {query, basic,mvc,bind,collection,$$,ScopicControl ,reflection,ScopicCommand} from "./../../js/corelib";
...
export module Material {
    ...
    function getSTemplate() {
        return _stemplate || (_stemplate = mvc.MvcDescriptor.Get('templates.qui-3ditem'));
    }
    export class Canvas3D extends UI.TControl<Canvas3D> {
        ...
        constructor() {
            super('templates.qui-Canvas3D', UI.TControl.Me as any);
        }
        private count = 0;
    ...
    export class GalleryItem extends UI.ScopicTemplateShadow {
        ...
        constructor(private data?: any | bind.Scop,dom?:HTMLElement) {
            super(dom || getSTemplate().Create(), GalleryItem.getScop(data));
        }
        public setName(name: string, dom: HTMLElement, cnt: UI.JControl, e: bind.IJobScop) {
            this[name] = dom;
...
ScopicControl.register("Material.Canvas3D", (n, d, c, p, pc, cnt) => {
    return new Material.Canvas3D();
});
ScopicControl.register("Material.GalleryItem", (name, dom, currentScop, parentScop, parentControl, cnt) => {
    return new Material.GalleryItem(currentScop || parentScop, dom as HTMLElement);
});
window["C3D"] = () => {
    require('./../QUI/QUI', (x) => {
        window['Qui']();
